...
将普通用户登录后的首页改为面向 CDN 使用者的轻量控制台，提供资源概况、接入进度、站点状态和常用操作入口；保留 `/resources` 作为完整配置工作区。同时修复 `/pages` 页面调用错误 API 路径导致的 404。
...
- 首页只负责导航和状态摘要，资源变更继续由 `/resources` 与 `/pages` 页面完成。
