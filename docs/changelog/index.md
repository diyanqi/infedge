...
- 普通用户和管理员现在可以直接输入根域或子域创建 CDN 站点，系统自动归组并为该明确域名生成 TXT 所有权验证记录。
- DNS 账号新增阿里云、腾讯云和华为云 DNS-01 验证支持，各服务商按各自凭据格式录入并支持保存前测试连接，普通用户可以在自己的站点中使用 TLS 证书能力。
- 普通用户可以管理自己的 DNS 服务商账号，每个用户最多 5 个，账号数据按用户隔离，平台账号仍由管理员统一维护。
- 普通用户控制台支持直接管理根域或子域，并补充 Pages、TLS、WAF 和固定 CNAME 接入引导；套餐额度中的域名分组数量不再要求用户先手动创建根域。
- 增加注册邮箱域名白名单、邮箱别名归一化以及 DNS TXT 所有权验证能力。
- 支持管理员维护套餐、易支付多渠道和节点组月流量额度，普通用户可以购买套餐、管理自己的 CDN 资源并按套餐次数发布全站版本。
- 支持节点独立月流量上限；同一节点组内的节点共享节点组月流量额度。
- 支持按用户跨节点累计高速流量，超额后按套餐限速，并将用户额度与节点/节点组月度总额度同时下发到 Agent。
- 支持管理员创建一次性套餐兑换码，普通用户可在套餐页兑换一个月套餐；套餐页高速流量按 GB/月并保留两位小数展示。

### 改进

- 修复 Next.js 生产静态导出中的页面参数和页面组件导出问题，确保代理规则及系统配置页面能够正常构建发布。
- 域名配置流程改为验证完成后进入独立站点控制台，可配置多源站及权重、Pages、HTTPS/TLS、缓存和 WAF 策略。
- 普通用户管理站点时进入独立的站点控制台页面，按“概览、域名、源站、HTTPS/证书、缓存、安全防护”分页组织配置，接入页只负责添加域名与 DNS TXT 所有权验证，不再堆叠全部设置。
- 普通用户首页升级为 CDN 控制台，集中展示站点、域名、源站和 Pages 概况，并通过接入进度和快速操作引导完成常用配置。
- 普通用户 CDN 控制台改为服务总览、网站列表和站点控制台三级流程，左侧导航使用“网站安全加速”和“Pages”等产品语言，降低接入和配置的理解成本。
- 管理员添加根域或站点域名时默认视为已完成所有权验证，不再要求配置 DNS TXT 验证记录，普通用户的验证流程保持不变。
- 允许不同账号重复接入同一域名，不再要求域名全局唯一；只有已被代理规则占用的具体域名才会禁止其他账号新增和使用。
- 将系统 SMTP 设置拆分为登录用户名和独立发件邮箱，并将产品品牌更新为“无尽边缘”，标注由 Rain-kl/OpenFlare 二开。
- 访问日志增加资源 owner 归属和用户月度流量累计，发布次数改为并发安全的每日额度预占。
...
- 新版 Server Docker 镜像在首次启动和后续升级时自动执行待处理的关系数据库与 ClickHouse 迁移，避免再手动运行迁移脚本。

### 修复

- 修复普通用户和管理员访问 Pages 页面时因前端请求错误接口而提示“请求的资源不存在”的问题。
- 修复普通用户申请证书时 DNS 账号下拉列表未显示自己创建的账号的问题。
- 修复普通用户删除已接入的根域或子域时提示“根域下仍有域名”而无法删除的问题；删除域名分组会级联清理其下域名及对应站点配置。
- 修复易支付渠道字段映射错误导致 PostgreSQL 无法新增支付渠道的问题。
- 修复管理员节点组页面请求错误接口导致资源不存在的问题。
- 修正套餐超额限速的展示和编辑单位为 KB/s，避免普通用户看到不准确的 MB/s 数值。
