...
普通用户资源沿根域、子域和代理路由逐级授权。Server 在部署前校验资源 owner、域名验证状态和 CNAME 接入状态；Agent 只消费发布后的全局不可变版本。
...
| 普通用户首页与管理员看板分流 | `/` 根据用户身份展示普通用户服务总览或全局运维总览；普通用户从网站列表 `/resources` 进入站点控制台 `/resources/detail` 与配置页 `/resources/configure`，并且只调用 owner-scoped custom API，避免暴露管理员观测数据 |
