...
export default function ResourcesPage() {
  ...
  useEffect(() => {
    ...
    const zone =
      zones.data.find((item) => item.id === requestedZoneId) ??
      zones.data.find((item) => item.id === selectedZoneId) ??
      zones.data[0];
    ...
    <div className='w-full flex flex-col gap-6 py-6 px-1'>
      <div className='flex flex-wrap items-center justify-between gap-3'>
        <div className='flex items-center gap-2'>
          ...
          <h1 className='text-2xl font-semibold tracking-tight'>网站配置</h1>
        </div>
                ...
                <div className='grid gap-3 sm:grid-cols-2 xl:grid-cols-3'>
                  {selectedDomains.map((domain) => (
                    <Button
                      key={domain.id}
                      variant={
                        domain.id === selectedDomainId ? 'secondary' : 'outline'
                      }
                  ...
                  <div className='grid gap-4 md:grid-cols-2'>
                    ...
                    <div className='flex flex-col gap-2'>
                      ...
                      <Select
                        value={
                          childCertId ||
                          (selectedDomain.cert_id
                            ? String(selectedDomain.cert_id)
                            : '')
                        }
                    ...
                    <div className='grid gap-3 md:grid-cols-2'>
                      ...
                      <div className='flex flex-col gap-2'>
                        ...
                        <Input
                          ...
                          value={limitReqPerIP}
                          onChange={(event) =>
                            setLimitReqPerIP(event.target.value)
                          }
                          placeholder='留空使用全局默认'
                        />
                      </div>
                ...
                <CardContent className='flex flex-col gap-5'>
                  <div className='flex flex-col gap-2'>
                    ...
                    <div className='flex gap-2'>
                      <Input
                        ...
                        placeholder={`例如：保护 ${selectedDomain.domain}`}
                      />
                      <Button
                  ...
                  {selectedRoute ? (
                    ...
                    <p className='text-sm text-muted-foreground'>
                      请先保存源站和站点配置，再绑定防火墙规则。
                    </p>
                  )}
                  <div className='flex flex-col gap-2 border-t pt-4'>
                    <Label>全局规则（只读）</Label>
                    {(policies.data?.global_rules ?? []).map((rule) => (
                      <div
                        key={rule.id}
                    ...
                    {!policies.data?.global_rules?.length && (
      ...
      <Card>
        <CardHeader>
          <CardTitle className='text-base'>全局策略（只读）</CardTitle>
          <CardDescription>
            管理员配置的默认限流对所有用户生效，普通用户不能修改。
          </CardDescription>
        </CardHeader>
        <CardContent className='grid gap-3 md:grid-cols-3'>
          <div className='flex items-center gap-2 text-sm'>
            ...
            单连接默认限速：
            <strong>{policies.data?.default_limit_rate || '未设置'}</strong>
          </div>
          <div className='flex items-center gap-2 text-sm'>
            <ShieldCheck className='size-4 text-primary' />单 IP 默认请求限流：
            <strong>
              {policies.data?.default_limit_req_per_ip || '未设置'}
            </strong>
          </div>
          <div className='flex items-center gap-2 text-sm'>
            <MapPin className='size-4 text-primary' />
            固定 CNAME：
            <strong>{policies.data?.cname || 'cname.edge.infvar.com'}</strong>
          </div>
        </CardContent>
      </Card>
    </div>
